...
namespace Math
{
    ...
    struct Matrix4 {
        ...
        static Matrix4<T> Scale(const Vector3<T>& koef)
        {
            Matrix4 m;
            m.x.x = koef.x; m.x.y = 0; m.x.z = 0; m.x.w = 0;
            m.y.x = 0; m.y.y = koef.y; m.y.z = 0; m.y.w = 0;
            m.z.x = 0; m.z.y = 0; m.z.z = koef.z; m.z.w = 0;
            m.w.x = 0; m.w.y = 0; m.w.z = 0; m.w.w = 1;
            return m;
        }
        ...
        static Matrix4<T> Rotate(T degrees, const Vector3<T>& axis)
        {
            T radians = degrees * 3.14159f / 180.0f;
            T s = std::sin(radians);
            T c = std::cos(radians);
